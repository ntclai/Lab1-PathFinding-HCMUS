...
class Map:
    ...
    def trace_path(self,has_path,dir_matrix):
        path=[]
        directions=['left','right','up','down']
        if has_path:
            fx,fy=self.G[0],self.G[1]
            path.append((fx,fy))
            while not(self.S[0]==fx and self.S[1]==fy):
                dir=dir_matrix[fy][fx]
                ...
                if fx==self.S[0] and fy==self.S[1]:
                    break
                path.append((fx,fy))
            path.reverse()
        return path
    ...
    def open(self,pygame,scroll_speed,video,image_path):
        if ('level_1' in image_path) and ('input2' in image_path):
            scroll_speed=3
        self.scroll_speed+=scroll_speed
        pygame.draw.rect(self.screen,Map.BLACK,((0-self.scroll_speed)*Map.cell_scale,0,(self.WIDTH//2)*Map.cell_scale,(self.HEIGHT)*Map.cell_scale))
        pygame.draw.rect(self.screen,Map.BLACK,((self.WIDTH//2+self.scroll_speed)*Map.cell_scale,0,(self.WIDTH)*Map.cell_scale,(self.HEIGHT)*Map.cell_scale))
        # self.writeVideo(video,image_path)
        if self.scroll_speed>=self.WIDTH:
            return True
        else:
            return False
    ...
    def run(self,has_path,cost,dir_matrix,pygame,index,algo,out_path,heuristic):
        clock = pygame.time.Clock()
        FPS = 15
        if ('level_1' in out_path) and ('input2' in out_path):
            FPS=300 
        self.frame_count=0
        ...
        if has_path:
            f.write(str(cost))
            finded_path=self.trace_path(has_path,dir_matrix)
        else:
            f.write('No Path')
            finded_path=[]
        f.close()
        ...
        while run:
            ...
            #     if self.open(pygame,scroll_speed,video,image_path):
            #         check_open=False
            #         self.scroll_speed=0
            #     self.writeVideo(video,image_path)
