...
#điều kiện để quyết định đường đi là heuristic

INF=int(1e9)

def gbfs(s, g, w, h, walls,h_option):
            ...
            if x in range(w) and y in range(h):
                if (x,y) not in walls:
                    if (x,y) not in opened:
                        if h_option==1:
                            cost_matrix[y][x] = heuristic.ManhattanDistance((x,y),g)+cost_matrix[point[1][1]][point[1][0]]
                        if h_option==2:
                            cost_matrix[y][x] = heuristic.EuclideanDistance((x,y),g)+cost_matrix[point[1][1]][point[1][0]]
                        if h_option==3:
                            cost_matrix[y][x] = heuristic.BreakingTie(s,(x,y),g)+cost_matrix[point[1][1]][point[1][0]]
                        dir_matrix[y][x] = i
