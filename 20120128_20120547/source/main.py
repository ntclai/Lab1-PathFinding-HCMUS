...
for l in level:
    ...
    if len(files)>0:
        ...
        name_empty_fold=input_path.replace('input','output')+l
        if not os.path.exists(name_empty_fold):
            os.makedirs(name_empty_fold)
...
for i in inputs:
    if 'level_1' in i:
        for j in range(0,9):
            ...
            if j==0:
                ...
                m.run(has_path,cost,dirs,[[m.S,m.G]],pygame,index,algo,out_path+algo,heuristic)
            if j==1:
            ...
            if j==3:
                ...
                m.run(has_path,cost,dirs,[[m.S,m.G]],pygame,index,algo,out_path+algo[0:4],heuristic)
            if j==4:
            ...
            if j==6:
                ...
                m.run(has_path,cost,dirs,[[m.S,m.G]],pygame,index,algo,out_path+algo[0:6],heuristic)
            if j==7:
    ...
    if 'level_2' in i:
        ...
        m.load(i)
        out_path='../output'+i[8:-4]+'/'
        has_path,cost,dirs,s_g=algo1(m.S,m.G,m.bonus,m.bonus_vals,m.WIDTH,m.HEIGHT,m.walls)
        index,algo,heuristic=int(i[-5]),'algo1',''
        m.run(has_path,cost,dirs,s_g,pygame,index,algo,out_path+algo,heuristic)
